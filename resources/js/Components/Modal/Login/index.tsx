...
const LoginModal: React.FC<LoginModalProps> = ({ onClose }) => {
    ...
    const isCodeDisabled = !code.trim();

    // Store the token and run the success animation + role-based redirect.
    const completeLogin = (data: any) => {
        ...
        const target = data.redirect || "/staff/dashboard";
        setTimeout(() => {
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            if (data.two_factor_required) {
                setTwoFactorMethod(data.method === "email" ? "email" : "totp");
                setResendNotice("");
    ...
    const handleVerifyCode = async () => {
        ...
        try {
            ...
            const response = await axios.post("/login/two-factor-challenge", payload);
    ...
    const handleResendCode = async () => {
        ...
        setError("");
        try {
            await axios.post("/login/two-factor-resend");
    ...
    const handleKeyPress = (e: React.KeyboardEvent) => {
        ...
        if (twoFactorRequired) {
            if (!isCodeDisabled && !isLoading) handleVerifyCode();
                ...
                {!isSuccess && !twoFactorRequired ? (
                    ...
                    >
                        ...
                        <div className="flex flex-col gap-2 mb-8">
                            <h2 className="text-3xl font-black text-gray-900 leading-tight">
                                Two-Factor <br />
                                <span className="text-green-800">Verification</span>
                            </h2>
                            <p className="text-gray-500 font-medium">
                                {recoveryMode
                        ...
                        <div className="space-y-6">
                            <div className="flex flex-col gap-2">
                                ...
                                <input
                                    ...
                                    onKeyPress={handleKeyPress}
                                    disabled={isLoading}
                                    className="w-full px-5 py-4 rounded-2xl bg-white border-2 border-gray-100 focus:border-green-800 focus:ring-4 focus:ring-green-800/5 outline-none transition-all placeholder:text-gray-300 font-medium tracking-widest text-center text-lg"
                                />
                            </div>

                            <button
                                type="button"
                                disabled={isCodeDisabled || isLoading}
                                onClick={handleVerifyCode}
                                className={`w-full py-4 mt-2 rounded-2xl font-black tracking-widest uppercase transition-all duration-300 shadow-xl border-b-4 ${isCodeDisabled || isLoading
                                    ? "bg-gray-100 text-gray-400 border-gray-200 cursor-not-allowed"
                                    : "bg-green-800 text-white border-yellow-600 hover:bg-green-900 hover:-translate-y-1 active:translate-y-0"
                                    }`}
                            >
                            ...
                            <button
                                ...
                                className="w-full text-sm font-bold text-green-800 hover:text-green-900 transition-colors disabled:opacity-50"
                            >
                                {recoveryMode
                            ...
                            {twoFactorMethod === "email" && !recoveryMode && (
                                <button
                                    type="button"
                                    disabled={isLoading}
                                    onClick={handleResendCode}
                                    className="w-full text-sm font-bold text-gray-500 hover:text-gray-700 transition-colors disabled:opacity-50"
                                >
