"""Generate a copy-paste-ready Word document containing the new
'b. Data Dictionary' subsection for insertion into the PROSIA capstone
paper, between section 'a. Entity Relationship Diagram' and the
renumbered section 'c. System Evaluation and Measurements'.

Output: docs/PROSIA_Data_Dictionary_Insert.docx
"""
...
def add_subsection_label(doc, text):
    p = doc.add_paragraph()
    run = p.add_run(text)
    run.bold = True
    run.font.size = Pt(12)
...
def add_body(doc, text, indent_cm=1.0):
    p = doc.add_paragraph()
    run = p.add_run(text)
    run.font.size = Pt(11)
...
def add_table(doc, headers, rows):
    ...
    widths = [Cm(3.4), Cm(2.6), Cm(3.6), Cm(1.1), Cm(1.7), Cm(4.4)]
...
LEAD_PARAGRAPHS = [
    "A data dictionary is a structured reference that defines every attribute of "
    "every entity in the database, specifying the data type, constraints, "
    "nullability, default value, and intended purpose of each field. While the "
    "Entity Relationship Diagram in Figure 6 establishes how the system's "
    "entities relate to one another, the data dictionary defines how each entity "
    "is implemented at the field level — providing the precise specification "
    "used during the database migration phase of development.",

    "The Data Dictionary of the AI-Powered Legal Document Management System was "
    "derived directly from the Business Rules presented in Table 2 and the Data "
    "Model presented in Table 3. Each table in the dictionary corresponds to one "
    "entity in the ERD, and the constraints listed for each attribute enforce the "
    "cardinality, referential integrity, and lifecycle requirements established "
    "by the business rules. For example, the ON DELETE CASCADE constraint on "
    "document_embeddings.doc_id enforces BR5 by ensuring that embeddings cannot "
    "exist independently of their parent document, while the nullable folder_id "
    "attribute in the documents table operationalizes BR11 by permitting "
    "documents to exist temporarily without a folder assignment during AI "
    "processing. The complete Data Dictionary is presented in Tables 4.1 through "
    "4.9 below.",
]


CLOSING_PARAGRAPHS = [
    "Tables 4.1 through 4.9 collectively define the physical schema of the "
    "AI-Powered Legal Document Management System. The users table (Table 4.1) "
    "consolidates authentication credentials, role classification, and the five "
    "granular permission flags (can_view, can_upload, can_edit, can_delete, "
    "can_archive) that drive the role-based access control module. The folders "
    "table (Table 4.2) implements the recursive hierarchy described in BR4 "
    "through a self-referencing parent_folder_id foreign key, enabling subfolders "
    "to be nested to any depth. The documents table (Table 4.3) carries the "
    "AI-generated metadata fields — title, description, ai_suggested_folder — "
    "alongside the file_hash attribute that enforces SHA-256 duplicate detection, "
    "and the physical_location attribute that supports the dual-tracking of "
    "digital and physical document copies. The document_embeddings table "
    "(Table 4.4) stores the 384-dimensional semantic vectors that power the "
    "AI-powered natural language search, with each record linked to its source "
    "document through a cascading foreign key. The ai_conversation and "
    "ai_histories tables (Tables 4.5 and 4.6) preserve the full record of every "
    "AI chat session, including the user's question, the AI-generated answer, "
    "and the document references cited in the response. The activity_logs table "
    "(Table 4.7) captures the audit trail required for accountability and "
    "traceability across all user actions. Finally, the notifications and "
    "permission_requests tables (Tables 4.8 and 4.9) implement the access "
    "management workflow that connects staff members to the administrator "
    "through structured request, review, and notification cycles.",

    "The Data Dictionary serves as the authoritative reference for the system's "
    "database implementation, ensuring that the constraints established at the "
    "conceptual level — through the business rules and the entity relationship "
    "diagram — are enforced at the physical level through column-level data "
    "types, nullability rules, default values, and foreign key constraints.",
]
...
def main():
    ...
    title_run = title_p.add_run("PROSIA — Data Dictionary Insert")
    title_run.bold = True
    ...
    sub_run = sub_p.add_run(
        "Ready-to-paste content for Chapter 5, Section 5.1 of the capstone paper"
    )
    sub_run.italic = True
    ...
    add_instruction_box(
        doc,
        "HOW TO USE: Copy everything below this box into your PROSIA paper. "
        "Place it immediately after Figure 6 (Entity Relationship Diagram) and "
        "before the existing subsection 'b. System Evaluation and Measurements'. "
        "After pasting, rename the existing subsection 'b.' to 'c.' so the "
        "section order becomes: a. Entity Relationship Diagram → b. Data "
        "Dictionary → c. System Evaluation and Measurements."
    )

    # ---- Begin paste region ----
    add_subsection_label(doc, "b.\tData Dictionary")
    ...
    for spec in TABLES:
        ...
        add_table_intro(doc, spec["intro"])
    ...
    # Stub showing the renumbered next subsection
    add_subsection_label(doc, "c.\tSystem Evaluation and Measurements")
    stub = doc.add_paragraph()
    stub_run = stub.add_run(
        "[ The existing 'b. System Evaluation and Measurements' content from "
        "the original paper continues here — only its letter label changes "
        "from 'b.' to 'c.' ]"
    )
    stub_run.italic = True
    stub_run.font.size = Pt(10)
    stub_run.font.color.rgb = RGBColor(0x80, 0x80, 0x80)
    stub.paragraph_format.left_indent = Cm(1.0)

    out = r"d:\capstone\Legal_Arch_aiu\docs\PROSIA_Data_Dictionary_Insert.docx"
    doc.save(out)
    print(f"Saved: {out}")
